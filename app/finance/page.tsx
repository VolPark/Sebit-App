...
import { Transaction } from '@/lib/types/finance-types'
...
export default function FinancePage() {
  ...
  async function handleSaveTransaction(data: any) {
    let success = false;
    if (data.id) {
      success = await updateTransaction(data.id, data);
      if (success) setStatusMessage('Transakce upravena');
    } else {
      success = await createTransaction(data);
      if (success) setStatusMessage('Transakce přidána');
    }

    if (!success) setStatusMessage('Chyba při ukládání');
  }
  ...
  async function handleDelete(id: number) {
    ...
    if (success) setStatusMessage('Transakce smazána');
    else setStatusMessage('Nepodařilo se smazat transakci');
  }
    ...
    <div className="p-4 sm:p-8 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="bg-green-50 dark:bg-green-900/20 p-6 rounded-2xl border border-green-100 dark:border-green-900/50 shadow-sm relative overflow-hidden group">
          ...
          <p className="text-3xl font-bold text-gray-900 dark:text-white">{currency.format(stats.income)}</p>
        </div>
        <div className="bg-red-50 dark:bg-red-900/20 p-6 rounded-2xl border border-red-100 dark:border-red-900/50 shadow-sm relative overflow-hidden group">
          ...
          <p className="text-3xl font-bold text-gray-900 dark:text-white">{currency.format(stats.expense)}</p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        ...
        <div className="w-full sm:w-auto">
          <select
            ...
            onChange={e => setFilters({ ...filters, divisionId: Number(e.target.value) || null })}
            ...
            <div className="flex justify-between items-start">
              ...
              <div className="text-right">
                <div className={`font-bold ${t.typ === 'Příjem' ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                  {t.typ === 'Příjem' ? '+' : '-'}{currency.format(Number(t.castka))}
                </div>
                <div className="flex gap-3 mt-2 justify-end">
                  ...
                  <button onClick={() => handleDelete(t.id)} className="text-xs font-medium text-red-500 hover:text-red-700 dark:hover:text-red-400">Smazat</button>
                </div>
              </div>
            </div>
          </div>
                  ...
                  >
                    <Menu.Items className="absolute right-0 top-full mt-1 w-40 origin-top-right rounded-lg bg-white dark:bg-slate-900 shadow-xl ring-1 ring-black/5 dark:ring-slate-700 focus:outline-none z-10">
                      <div className="p-1">
                        ...
                        <Menu.Item>
                          {({ active }) => (
                            <button onClick={() => handleDelete(t.id)} className={`${active ? 'bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400' : 'text-red-600 dark:text-red-400'} group flex w-full items-center rounded-md px-2 py-2 text-sm`}>
