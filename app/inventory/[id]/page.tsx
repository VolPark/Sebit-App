                        ...
                        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 px-4 py-2 rounded-xl text-right min-w-[200px]">
                            ...
                            <div className="space-y-1">
                                {item.stocks.map(stock => (
                                    stock.quantity > 0 && (
                                        <div key={stock.center_id} className="flex justify-between text-sm">
                                            <span className="text-gray-500">{stock.centers?.name}:</span>
                                            <span className="font-bold">{stock.quantity} {item.unit}</span>
                                        </div>
                                    )
                                ))}
                            </div>
                        ...
                        <table className="w-full text-left text-sm">
                            <thead className="bg-gray-50 dark:bg-slate-950/50 border-b border-slate-200 dark:border-slate-800">
                                <tr>
                                    ...
                                    <th className="px-6 py-4 font-semibold text-gray-900 dark:text-gray-200">Středisko</th>
                                ...
                                {movements.length === 0 ? (
                                    ...
                                    movements.map(m => (
                                        <tr key={m.id} className="hover:bg-gray-50/50 dark:hover:bg-slate-800/50">
                                            ...
                                            <td className={`px-6 py-4 text-right font-bold ${m.quantity_change > 0 ? 'text-green-600' : m.quantity_change < 0 ? 'text-orange-600' : 'text-gray-600'}`}>
                                            ...
                                            <td className="px-6 py-4 text-sm text-gray-600 dark:text-gray-400">
                                                {m.type === 'TRANSFER' ? (
                                                    <div className="flex items-center gap-1">
                                                        ...
                                                        <span>→</span>
                                                        <span className="font-bold text-gray-900 dark:text-white">{m.target_centers?.name}</span>
                                                    </div>
                                                ) : (
