...
export async function getDashboardDataBeta(
      ...
      if (doc.type === 'sales_invoice') {
        ...
        if (doc.mappings && doc.mappings.length > 0) {
          for (const m of doc.mappings) {
            ...
            const linkedAkce = m.akce; // Use joined data from query
  ...
  if (!useFilterMode) {
    ...
    workerMonthTotalCost.forEach((laborCost, rateKey) => {
      ...
      // CRITICAL FIX: Check if this month is within the selected period
      if (d < startDate || d > endDate) {
        return; // Skip months outside the period
      }
  ...
  // 7. Top Clients & Workers (Aggregated from Monthly Stats to ensure consistency)
  // Instead of re-calculating from raw data (which led to discrepancies), we aggregate the already processed monthly stats.
  const globalClientMap = new Map<number, { name: string, total: number }>();

  monthlyClientStats.forEach((cMap) => {
    cMap.forEach((stats, clientId) => {
      const curr = globalClientMap.get(clientId) || { name: stats.name, total: 0 };
      curr.total += stats.total;
      globalClientMap.set(clientId, curr);
    });
  });

  const topClients = Array.from(globalClientMap.entries())
...
export async function getDetailedStatsBeta(
  ...
  if (accountingDocs.length > 0) {
    ...
    accountingDocs.forEach((doc: any) => {
      ...
      if (
        ...
        desc.includes('převod mezi vlastními účty') ||
        desc.includes('peníze na cestě') ||
        (desc.includes('převod') && desc.includes('účt'))
      ) {
        return;
      }
  ...
  fixedCosts.forEach((fc: any) => {
    const key = `${fc.rok}-${fc.mesic - 1}`;  // 0-indexed (DB has 1-indexed)
    const amount = Number(fc.castka) || 0;
    if (fc.division_id) {
  ...
  globalPrace.forEach((p: any) => {
    const d = new Date(p.datum);
    const key = `${d.getFullYear()}-${d.getMonth()}`;  // 0-indexed
    monthlyGlobalHours.set(key, (monthlyGlobalHours.get(key) || 0) + (Number(p.pocet_hodin) || 0));
  });
  ...
  globalPrace.forEach((p: any) => {
    if (p.pracovnik_id) {
      const d = new Date(p.datum);
      const key = `${p.pracovnik_id}-${d.getFullYear()}-${d.getMonth()}`;  // 0-indexed
  ...
  // 1. From Mzdy
  mzdy.forEach((m: any) => {
    const key = `${m.pracovnik_id}-${m.rok}-${m.mesic - 1}`;  // 0-indexed (DB has 1-indexed)
    workerMonthTotalCost.set(key, (workerMonthTotalCost.get(key) || 0) + (Number(m.celkova_castka) || 0));
  });
  // 2. From Mapped Labor
  workerMonthlyMappedLabor.forEach((amount: number, key: string) => {
  ...
  filteredPrace.forEach((p: any) => {
    ...
    // Labor Cost
    const rateKey = `${p.pracovnik_id}-${d.getFullYear()}-${d.getMonth()}`;  // 0-indexed
    ...
    // Overhead Cost
    const overheadKey = `${d.getFullYear()}-${d.getMonth()}`;  // 0-indexed
  ...
  const clientStats: ClientStats[] = clients.map((c: any) => {
    ...
    const actions: ActionStats[] = cAkce.map((a: any) => {
      ...
      let additionalCosts = 0; // Non-material, non-labor costs from accounting
      ...
      if (!CompanyConfig.features.enableAccounting) {
        ...
        // so we take it from action data even when accounting is enabled
        aMaterialRevenue = Number(a.material_klient) || 0;
      }

      const aLaborCost = actionLaborCosts.get(a.id) || 0;
      const aOverheadCost = actionOverheadCosts.get(a.id) || 0;
      const aTotalCost = aMaterialCost + aLaborCost + aOverheadCost + additionalCosts;
