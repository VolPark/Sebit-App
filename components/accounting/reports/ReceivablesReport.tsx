...
export function ReceivablesReport() {
    const [groupedData, setGroupedData] = useState<GroupedReceivables[]>([]);
    ...
    const fetchReceivables = async () => {
        setLoading(true);
        try {
            // Fetch sales invoices with positive amount
            // We'll filter unpaid/partial client-side or we can improve query if needed
            ...
            // Fetch mappings for these documents to have complete data for Detail Modal
            ...
            // Group by supplier
            const groups: { [key: string]: GroupedReceivables } = {};
            let totalAmount = 0;
            let totalRemaining = 0;

            unpaidDocs.forEach(doc => {
                const name = doc.supplier_name || 'Neznámý odběratel';
                if (!groups[name]) {
                    groups[name] = {
                        supplier_name: name,
                        documents: [],
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm">
                <div className="flex flex-col md:flex-row justify-between md:items-start gap-4 mb-6">
                    <div>
                        <div className="flex items-center gap-2 mb-2">
                            ...
                            <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Pohledávky</h1>
                        </div>
                    ...
                    <div className="flex flex-col items-end gap-4">
                        ...
                        <div className="text-right space-y-2 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-100 dark:border-slate-800">
                            ...
                            <div className="flex justify-between gap-8 text-sm pt-2 border-t border-slate-200 dark:border-slate-700">
                                <span>Dlužná částka celkem:</span>
                ...
                {/* Table Header (Visual only, columns aligned with rows below) */}
                        ...
                        <div key={idx} className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
                            ...
                            <div className="px-6 py-4 bg-slate-50/50 dark:bg-slate-800/30 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center">
                                ...
                                <div className="text-right">
                                    <span className="text-xs text-slate-500 uppercase tracking-wider mr-2">Celkem dluží</span>
                            ...
                            <div className="divide-y divide-slate-100 dark:divide-slate-800/50">
                                {group.documents.map(doc => {
                                    // Helper: get raw data for extra fields
                                    ...
                                    return (
                                        ...
                                        >
                                            <div className="md:col-span-2 font-medium text-blue-600 dark:text-blue-400 group-hover:underline decoration-1 underline-offset-2 flex items-center gap-2">
                                                {doc.number}
                                            </div>

                                            {/* Mobile-only labels could be added here if we wanted fully responsive grid, but for now keeping compatible with desktop structure */}
