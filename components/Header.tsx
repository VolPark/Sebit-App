...
function NavLink({ href, children, pathname }: { href: string, children: React.ReactNode, pathname: string }) {
  ...
  return (
    <Link
      href={href}
      className={`px-5 py-2 rounded-full text-sm transition-all duration-300 ${isActive ? 'bg-white text-[#E30613] shadow-md ring-1 ring-black/5 font-bold scale-105' : 'font-medium text-gray-500 hover:text-gray-900 hover:bg-white/50'}`}
    >
      {children}
    </Link>
  )
...
export default function Header() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 p-2 sm:p-4 flex justify-center">
      <nav role="navigation" aria-label="Hlavní navigace" className="relative bg-white/90 backdrop-blur-xl border border-white/20 shadow-lg shadow-slate-200/50 rounded-2xl sm:rounded-full px-3 sm:px-6 py-2.5 flex items-center justify-between w-full max-w-7xl transition-all">
        ...
        <div className="flex items-center gap-3">
          <Link href="/" className="flex items-center gap-2 group">
            ...
            <span className="text-lg font-bold tracking-tight text-gray-900 hidden sm:block group-hover:text-[#E30613] transition-colors">
              Interiéry Horyna
            </span>
          </Link>
        </div>
        ...
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-full border-[3px] border-white shadow-sm overflow-hidden hidden sm:block bg-gray-100 ring-1 ring-gray-200">
            {userImage ? (
